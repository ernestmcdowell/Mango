package mango.terrain;

import mango.ObjectLoader;
    ...
    private Model generateTerrain(ObjectLoader loader) {
        ...
        for(int i = 0; i < VERTEX_COUNT; i++){
            for(int j = 0; j < VERTEX_COUNT; j++){
                ...
                textureCoord[vertexPointer * 2] = j / (VERTEX_COUNT - 1.0f) * SIZE;
                textureCoord[vertexPointer * 2 + 1] = i / (VERTEX_COUNT - 1.0f) * SIZE;
                vertexPointer++;
            }
        }
        int pointer = 0;
        ...
        return loader.loadModel(vertices, textureCoord, normals, indices);
    }
